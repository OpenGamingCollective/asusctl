...
use rog_platform::platform::{GpuMode, PlatformProfile, Properties};
...
impl Profile {
    ...
    pub fn fan_curve_data(&self, _p: PlatformProfile) -> Result<Vec<CurveData>> {
        Ok(vec![CurveData::default()])
    }

    pub fn set_fan_curve(&self, _p: PlatformProfile, _c: CurveData) -> Result<()> {
        Ok(())
    }
...
impl Supported {
    pub fn supported_properties(&self) -> Result<Vec<Properties>> {
        Ok(vec![
            Properties::ThrottlePolicy,
